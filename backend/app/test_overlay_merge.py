...
def _pick_first_n_main_files_with_overlay(
    memories_dir: Path, n: int, overlay_idx: dict[str, list[str]]
) -> list[Path]:
    if not memories_dir.is_dir():
        return []
    files: list[Path] = []
...
def main() -> int:
    ...
    for i, main_path in enumerate(mains, start=1):
        ...
        if out and os.path.isfile(out):
            ...
            print(
                f"[{i:02d}] main={main_path.name}  overlay={os.path.basename(overlay)}  -> OK combined={out_name}"
            )
        else:
            failed += 1
            print(
                f"[{i:02d}] main={main_path.name}  overlay={os.path.basename(overlay)}  -> FAIL (combine returned None)"
            )
