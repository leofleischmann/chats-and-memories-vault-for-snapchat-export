...
Die `.env` liegt im Repo; Werte bei Bedarf anpassen.
...
1. **Get your Snapchat export**  
   ...
   ![Snapchat – Export anfordern](images/export-all.png)

2. **Put ZIPs into the folder**  
   Copy all downloaded ZIP files into **`input zip/`** in this project.

   ![ZIP-Dateien in input zip/ ablegen](images/export-in-folder.png)

3. **Run import**  
   Open `http://localhost:5173` → **Dashboard** → **Data management**, then:

   - **Unpack ZIP → input + import ** – unpacks ZIPs into `input/` (chat_media, memories, JSON, etc.) and starts the import process.
...
1. Put the new ZIP(s) into `input zip/` (you can delete older ZIPs).
